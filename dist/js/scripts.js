...
document.querySelectorAll('a:has(.macro--button)').forEach(item => item.removeAttribute('title'));
...
if(pageType === 'SF') {
    ...
    initStickyBar();
    document.querySelectorAll('.topic--description').forEach(el => el.innerHTML = el.innerHTML.replaceAll('{', '<tag-highlight>').replaceAll('}', '</tag-highlight>'));
}
...
if(pageType === 'ST') {
    ...
    $(".post + .activeuserstrip").nextAll().andSelf().wrapAll(`<div class="posts--info"></div>`);
    if(document.querySelector('.post.tindr')) {
        initTindr();
    } else if(document.querySelector('.post.insta')) {
        initInsta();
    ...
    $('#ST main > table:first-of-type + br + .tableborder').wrapAll(`<div class="posts"></div>`);
...
/********** Topic View **********/
if(pageType === 'Post') {
    let emojis = document.querySelector('#smilies-table').parentNode.innerHTML;
    document.querySelector('#code-buttons').insertAdjacentHTML('beforebegin', `<div id="smilies-table"></div>`);
    document.querySelector('div#smilies-table').innerHTML = emojis;
    ...
    const setUpCustomBBcodeButtons = () => {
        ...
        for (const group of bbcode) {
            ...
            cbHolder += `<div class="buttonHolder accordion--content ${group.collapsed ? "bbcCollapse" : "bbcNoCollapse"}"><div class="accordion--inner"><div class="buttons--flex">`
...
if(pageType === 'UserCP' || pageType === 'Msg') {
	...
	if($('body.code-01').length > 0 && pageType === 'UserCP') {
        ...
        let defaultSpecies = document.querySelector('#field_32_input').options[document.querySelector('#field_32_input').selectedIndex].value;
        let subspecies = document.querySelectorAll('#field_63_input option');
	    let hasSubs = 0;
        subspecies.forEach(el => {
            if(el.value.includes(`${defaultSpecies}-`) || el.value === 'unset') {
                el.classList.remove('hidden');
		        hasSubs++;
            } else {
                el.classList.add('hidden');
        ...
        if(hasSubs <= 1 && document.querySelector('#field_63_input option[value="na"]')) {
            document.querySelector('#field_63_input option[value="na"]').classList.remove('hidden');
        }
        document.querySelector('#field_32_input').addEventListener('change', e => {
            hasSubs = 0;
            let species = e.target.options[e.target.selectedIndex].value;
            subspecies.forEach(el => {
                if(el.value.includes(`${species}-`) || el.value === 'unset') {
                    el.classList.remove('hidden');
                hasSubs++;
                } else {
                    el.classList.add('hidden');
...
if(pageType === 'store') {
  ...
  document.querySelectorAll('#store.store-inventory #ucpcontent > table > tbody > tr:not(:first-child):not(:nth-child(2)):not(:nth-child(3)) td:nth-child(2)').forEach(item => {
      if (unusable.includes(item.innerText.toLowerCase())) {
          item.parentNode.querySelector('td:last-child').remove();
      }
  });
}
