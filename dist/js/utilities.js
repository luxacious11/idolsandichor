...
function fetchURL(json, sheetType = 'claims') {
    return `https://opensheet.elk.sh/${sheetID[sheetType]}/${json}`;
}
function setTheme() {
...
function initSwitcher() {
	...
	switchValues.forEach((character, i) => {
		let characterName = formatName(character.character, 'b').replace('(p)', 'primary account');
...
function rgbToHsl(r, g, b) {
    ...
    // Convert HSL values to their standard ranges
...
function initForums() {
    ...
    document.querySelectorAll('.forum .forum--image').forEach(image => {
        ...
        if(subforumEl) {
            image.closest('.forum').querySelector('.forum--location-links .scroll').insertAdjacentHTML('beforeend', subforumEl.innerHTML);
            subforumEl.remove();
        }
        if(linkSet) {
            image.closest('.forum').querySelector('.forum--location-links .scroll').insertAdjacentHTML('beforeend', linkSet.innerHTML);
        }
    });
...
function initDiscordTagging(location) {
    ...
    document.querySelector(location).insertAdjacentHTML('beforeend', `<div class="invisibleElTagging"></div><div class="alert-options">
        <button onClick="toggleAlerts(this)" class="macro--button" title="Tag in Discord"><i class="fa-light fa-sharp fa-tags"></i><i class="fa-solid fa-xmark"></i></button>
    ...
    document.querySelector('#sendAlert').addEventListener('click', e => {
	    ...
	    let message = ``;
        var includes = [...new Set(Array.from(document.querySelectorAll('.post')).map(item => item.dataset.character))];
        var characterList = ``;
        ...
        let triggerBlock = posts[posts.length - 1].querySelectorAll('.triggers');
        ...
        let noteBlock = posts[posts.length - 1].querySelectorAll('.notes');
...
function avatarPreviews(field) {
    let html = `<img src="${getValue(document.querySelector(`${field} input`))}" alt="Avatar" title="Avatar" />`;
    document.querySelector('.clip-avatar-image').innerHTML = html;
}
function aestheticPreviews() {
    if(document.querySelector('.clip-aesthetics')) {
        let topHtml = ``, bottomHtml = ``;
        aestheticImageFields.forEach((field, i) => {
            if(i > 3) {
                bottomHtml += `<img src="${getValue(document.querySelector(`${field} input`))}" alt="Aesthetic #${i + 1}" title="Aesthetic #${i + 1}" />`;
            } else {
                topHtml += `<img src="${getValue(document.querySelector(`${field} input`))}" alt="Aesthetic #${i + 1}" title="Aesthetic #${i + 1}" />`;
            }
        });
        document.querySelector('.clip-aesthetics .top-left').innerHTML = topHtml;
        document.querySelector('.clip-aesthetics .bottom-right').innerHTML = bottomHtml;
    }
}
...
function initTindr() {
    let staticClaims = [], staticMembers = [];
    fetch(fetchURL(claimsJson))
    .then((response) => response.json())
    .then((characterData) => {
        staticClaims = characterData;

        fetch(fetchURL(membersJson))
        .then((response) => response.json())
        .then((memberData) => {
            staticMembers = memberData;
        }).then(() => {
            let posts = document.querySelectorAll('.post');
            posts.forEach(post => {
                let account = post.dataset.account;
                let parent = post.dataset.parent;
                let existing = staticClaims.filter(item => item.AccountID === account)[0];
                if(existing) {
                    let parentAccount = staticMembers.filter(item => item.AccountID === existing.ParentID)[0];
                    post.dataset.author = parentAccount.Member;
                    post.querySelectorAll('.clip-player').forEach(clone => clone.innerHTML = initPlayerInfo(parentAccount));
                } else {
                    let parentAccount = staticMembers.filter(item => item.AccountID === account)[0];
                    if(!parentAccount) {
                        parentAccount = staticMembers.filter(item => item.AccountID === parent)[0];
                    }
                    if(parentAccount) {
                        post.dataset.author = parentAccount.Member;
                    } else {
                        post.dataset.author = 'undefined';
                    }
                    post.querySelectorAll('.clip-player').forEach(clone => clone.innerHTML = initPlayerInfo(parentAccount));
                }
            })
        
            if(document.querySelector('.post.tindr:has(tag-bio)')) {
                document.querySelector('.post.tindr:has(tag-bio)').classList.add('tindr--profile');
            }
            $('.post.tindr:not(:has(tag-bio))').nextUntil('.posts--info').andSelf().wrapAll('<div class="posts--socials-wrap"><div class="posts--socials-wrap-inner"></div></div>');
            if(document.querySelector('.post.tindr:has(tag-bio)')) {
                document.querySelector('.post.tindr:has(tag-bio) .social--name').innerText = document.querySelector('.maintitle .topic-title').innerText;
                document.querySelector('.post.tindr:has(tag-bio) .social--info').insertAdjacentHTML('beforeend', document.querySelector('.maintitle .topic-desc').innerHTML);
        
                if(!document.querySelector('.posts--socials-wrap')) {
                    document.querySelector('.post.tindr:has(tag-bio)').insertAdjacentHTML('afterend', `<div class="posts--socials-wrap"><div class="posts--no-posts">Oops, this is a new profile. There's nothing here!</div></div>`);
                }
            }
            $('.post.tindr--profile').nextUntil('.posts--info').andSelf().wrapAll('<div class="posts--socials"></div>');
            document.querySelectorAll('.posts--socials-wrap .post.tindr').forEach(post => {
                if(post.querySelector('tag-contact')) {
                    let profileURL = post.querySelector('tag-contact').dataset.thread;
                    let profileName = post.querySelector('tag-contact').innerText;
                    post.querySelector('.social--name').innerText = profileName;
                    post.querySelector('.social--name').setAttribute('href', profileURL);
                    post.querySelector('tag-contact').remove();
                }
            });
            var $grid = $('.posts--socials-wrap-inner').masonry({
                gutter: 15,
                percentPosition: true,
                itemSelector: '.posts--socials-wrap-inner .post.tindr'
            });
...
function initInsta() {
    ...
    .then((characterData) => {
        ...
        .then((memberData) => {
            ...
            if(document.querySelector('.post.insta:has(tag-bio)')) {
                document.querySelector('.post.insta:has(tag-bio)').classList.add('insta--profile');
            }
            $('.post.insta:not(:has(tag-bio))').nextUntil('.posts--info').andSelf().wrapAll('<div class="posts--socials-wrap"><div class="posts--socials-wrap-inner"><div class="scroll accent"></div></div></div>');
            if(document.querySelector('.post.insta:has(tag-bio)')) {
                document.querySelector('.post.insta:has(tag-bio) .social--name').innerText = document.querySelector('.maintitle .topic-title').innerText;
                document.querySelector('.post.insta:has(tag-bio) .social--info').insertAdjacentHTML('beforeend', document.querySelector('.maintitle .topic-desc').innerHTML);

                if(!document.querySelector('.posts--socials-wrap')) {
                    document.querySelector('.post.insta:has(tag-bio)').insertAdjacentHTML('afterend', `<div class="posts--socials-wrap"><div class="posts--no-posts">Oops, this is a new profile. There's nothing here!</div></div>`);
                }
            }
            $('.post.insta--profile').nextUntil('.posts--info').andSelf().wrapAll('<div class="posts--socials"></div>');
            document.querySelectorAll('.posts--socials-wrap .post.insta').forEach(post => {
                if(post.querySelector('tag-contact')) {
                    let profileURL = post.querySelector('tag-contact').dataset.thread;
                    let profileName = post.querySelector('tag-contact').innerText;
                    post.querySelector('.social--name').innerText = profileName;
            ...
            let $grid = $('.posts--socials-wrap-inner .scroll');
            $(document).ready(function() {
                $grid.masonry({
                    gutter: 15,
                    percentPosition: true,
                    itemSelector: '.posts--socials-wrap-inner .post.insta'
                });
            ...
            document.querySelectorAll('.social--controls a:has(.fa-pen)').forEach(editBtn => {
...
function initComms() {
    ...
    .then((characterData) => {
        ...
        .then((memberData) => {
            ...
            $('.post.comm').nextUntil('.posts--info').andSelf().wrapAll('<div class="posts--comms-wrap"><div class="posts--comms-wrap-inner"><div class="scroll accent"></div></div></div>'); 
            ...
            let commTopper = `<div class="comm--top">${contact}</div>`;
            let commEnder = `<div class="comm--bottom">
                <span></span>
                <button type="button" class="post--permalink" data-clipboard-text="[action]content[/action]" onclick="copiedClipboard(this)" title="New Action">
                    <i class="fa-light fa-sharp fa-microphone"></i>
                    <i class="fa-light fa-sharp fa-clipboard"></i>
                </button>
                <button type="button" class="post--permalink" data-clipboard-text="[msg]content[/msg]" onclick="copiedClipboard(this)" title="New Message">
                    <i class="fa-light fa-sharp fa-paper-plane-top"></i>
                    <i class="fa-light fa-sharp fa-clipboard"></i>
                </button>
            </div>`;
            document.querySelector('.posts--comms-wrap').insertAdjacentHTML('afterbegin', commTopper);
            document.querySelector('.posts--comms-wrap').insertAdjacentHTML('beforeend', commEnder);
            ...
            posts.forEach((post, i) => {
                if(i % 2 !== 0) {
                    post.querySelector('a.comm--name').innerHTML = contact.toLowerCase();
                }
            });
        });
    });
}
function copiedClipboard(e) {
...
function initPlayerInfo(parent = null) {
    if(parent) {
        return `<div class="items--item">
            ...
            <div class="items--item">
                <b>Please Avoid</b>
                <span class="avoid">${parent.Triggers}</span>
            </div>`;
    } else {
        return `<div class="items--item">
                <b>Alias</b>
            ...
            <div class="items--item">
                <b>Please Avoid</b>
                <span class="avoid">claims pending</span>
            </div>`;
    }
}
function initPosts() {
    ...
    .then((characterData) => {
        ...
        .then((memberData) => {
            ...
            posts.forEach(post => {
                let account = post.dataset.account,
                    character = post.dataset.character,
                    creatureType = post.dataset.creatureType,
                    subspecies = post.dataset.subspecies,
                    ancestry = post.dataset.ancestry,
                    parent = post.querySelector('.post--avatar').dataset.parent,
                    birthday = {
                ...
                post.querySelector('.post--topper > a').innerHTML = formatName(character, 'b')

                if(parseInt(parent) !== 0) {
                    post.querySelector('.post--links a:first-child').setAttribute('href', `?showuser=${parent}`);
                    switch(ancestry) {
                        case 'Deity':
                            post.querySelectorAll('.ancestryCheck:not(.deityOnly)').forEach(el => el.remove());
                            break;
                        case 'Hero':
                            post.querySelectorAll('.ancestryCheck:not(.heroOnly)').forEach(el => el.remove());
                            break;
                        case 'Creature':
                            if(creatureType.toLowerCase() === 'beast') {
                                post.querySelectorAll('.ancestryCheck:not(.beastOnly):not(.creatureOnly)').forEach(el => el.remove());
                            } else if(creatureType.toLowerCase() === 'spirit') {
                                post.querySelectorAll('.ancestryCheck:not(.spiritOnly):not(.creatureOnly)').forEach(el => el.remove());
                            } else {
                                post.querySelectorAll('.ancestryCheck:not(.creatureOnly)').forEach(el => el.remove());
                            }
                            if(subspecies.toLowerCase() === 'unset' && subspecies.toLowerCase() === 'not applicable' && subspecies.toLowerCase() === '') {
                                post.querySelectorAll('.hasSubspecies').forEach(el => el.remove());
                            }
                            break;
                        case 'Mortal':
                            post.querySelectorAll('.ancestryCheck:not(.mortalOnly)').forEach(el => el.remove());
                            break;
                        default:
                            post.querySelectorAll('.ancestryCheck').forEach(el => el.remove());
                            break;
                    }

                    if(birthday.year && birthday.month && birthday.day) {
                        post.querySelector('age-clip').innerHTML = calculateAge(birthday);
                    } else {
                        post.querySelector('age-clip').innerHTML = 'unset';
                    }
                }
            });
        });
...
function populatePage(array) {
    ...
    members.forEach(member => {
        document.querySelector('.members--filter-group[data-filter-group="alias"] .accordion--inner').insertAdjacentHTML('beforeend', `<label><input type="checkbox" value=".${member.clean}"/>${member.raw}</label>`);
    });
}
...
function sendAjax(form, data, staffDiscord, publicDiscord, async = true) {
    ...
    $.ajax({
        url: `https://script.google.com/macros/s/${data.DeployID}/exec`,   
        data: data,
        method: "POST",
        type: "POST",
        async: async,
...
function addRow(e) {
    ...
    } else if(e.closest('.multi-buttons').dataset.rowType === 'subtypes') {
        e.closest('.adjustable').querySelector('.rows').insertAdjacentHTML('beforeend', formatTextFields('subtype'));
...
function formatSectionFields() {
    return `<div class="section-wrap row" data-type="grid" data-gap="md">
...
function formatRoleFields() {
    return `<div class="section-role row" data-type="grid" data-columns="2" data-gap="md">
...
function formatTextFields(type) {
    return `<div class="${type}-row row textfield-row" data-type="grid">
        <label class="${type}">
            <span><input type="text" placeholder="${type}" /></span>
        </label>
    </div>`;
}
function formatSubspeciesFields() {
    return `<div class="subspecies-row row" data-type="grid" data-gap="md">
        <label class="subspecies">
            <span><input type="text" placeholder="Subspecies" /></span>
        </label>
...
function formatJobChanges(data) {
    ...
    jobs.forEach(job => {
        html += `<div data-employer="${cleanText(job.employer)}" data-section="${cleanText(job.section)}" data-position="${cleanText(job.position)}" data-type="grid" data-gap="sm" class="job-row">
            ...
            <div data-type="grid" data-columns="2" data-gap="md">
                <label class="job-section">
                    <div class="h7">Section</div>
                    <span><input type="text" placeholder="${job.section}"></span>
                </label>
                <label class="position">
                    <div class="h7">Position</div>
...
function setEmployers(data, formID, wrapClass) {
    data.sort((a, b) => {
        if (a.Hiring.toLowerCase() === 'yes' && b.Hiring.toLowerCase() !== 'yes') {
            ...
            return 1;
        } else if (a.Hiring.toLowerCase() === 'ask first' && b.Hiring.toLowerCase() !== 'ask first' && b.Hiring.toLowerCase() !== 'yes') {
            return -1;
        } else if (a.Hiring.toLowerCase() !== 'ask first' && a.Hiring.toLowerCase() !== 'yes' && b.Hiring.toLowerCase() === 'ask first') {
            return 1;
    ...
    data.forEach((employer, i) => {
        if(i === 0) {
            ...
            } else if(employer.Hiring.toLowerCase() === 'no') {
                optgroup = `Not Hiring`;
            }
...
function setList(fieldClass, data, type) {
    document.querySelectorAll(fieldClass).forEach(el => {
        ...
        if(type === 'members' && el.classList.contains('business-members')) {
            html += `<option value="0" class="staffOnly">Staff</option>`;
        }
        ...
        data.forEach(item => {
            switch(type) {
                case 'businesses':
                    html += `<option value="${cleanText(item.Employer)}">${capitalize(item.Employer, [' ', '-'])}</option>`;
                    break;
                case 'subtypes':
                    html += `<option value="${item.value}" class="hidden">${capitalize(item.title, [' ', '-'])}</option>`;
                    break;
                case 'species':
                    html += `<option value="${item.Creature}">${capitalize(item.Creature, [' ', '-'])}</option>`;
                    break;
                case 'members':
                    html += `<option value="${item.AccountID}">${capitalize(item.Member, [' ', '-'])}</option>`;
                    break;
                case 'plots':
                    html += `<option value="${item.PlotID}">${capitalize(item.Plot, [' ', '-'])}</option>`;
                    break;
                case 'plotsections':
                    html += `<option value="${item.title}">${capitalize(item.title, [' ', '-'])}</option>`;
                    break;
                case 'plotroles':
                    html += `<option value="${item.role}">${capitalize(item.role, [' ', '-'])}</option>`;
                    break;
                case 'canons':
                    html += `<option value="${item.Canon}">${capitalize(item.Canon, [' ', '-'])}</option>`;
                    break;
                case 'characters':
                case 'pending':
