/***** Profile *****/
function formatAesthetics(images, index) {
    let imageHTML = ``;
    images.forEach((image) => {
        imageHTML += `<img src="${image}" title="Image #${index}" alt="Image #${index}" loading="lazy" />`;
        index++;
    })
    return imageHTML;
}
function setRoster() {   
    ...
    alphaChars.forEach(character => {
        ...
        let html = `<a class="switch--block" href="?showuser=${character.account}">
            ${imageDiv}
            <span class="switch--name">${formatName(capitalize(character.character), 'b')}</span>
        </a>`;
...
function initCharacter(images, overflow, title, birthday, speciality, isLocal = false) {
    ...
    //set up aesthetics
    document.querySelector('.profile--aesthetics.top-left').innerHTML = formatAesthetics(images.splice(4, 7), 1);
    document.querySelector('.profile--aesthetics.bottom-right').innerHTML = formatAesthetics(images.splice(0, 4), 5);
...
function cpShift() {
	...
	if(account.toLowerCase() == 'character') {
        ...
        showHeaders = [...allHeaders, ...memHeaders];
...
function formatMemberRow(type, data, extraFilters = '') {
    ...
    if(type === 'character') {
        tagList += `${data.character.ageClass} ${data.character.ancestry}`;
        info += `<div class="member--stats">
            <span>${data.character.age} years old</span>
            <span>${data.character.pronouns}</span>
            <span>${data.character.ancestry}</span>
            ${data.character.detail1 ? `<span>${data.character.detail1}</span>` : ``}
            ${data.character.detail2 ? `<span>${data.character.detail2}</span>` : ``}
        </div>`;
        details = `<span>Played by ${data.writer.alias}</span>
        <span>Joined ${data.universal.dates.joined}</span>`;
    } else {
        info += `<div class="member--stats">
        ...
        details += `<span>Joined ${data.universal.dates.joined}</span>
        <span>Last seen ${data.universal.dates.lastActive}</span>`;
    }
    return `<div class="members--member grid-item g-${data.universal.groupID} ${data.writer.aliasClass} ${type} ${extraFilters} ${tagList}">
        <div class="member">
            ...
            <div class="member--main">
                <div class="member--title"><a href="?showuser=${data.universal.id}">${data.universal.name}</a></div>
